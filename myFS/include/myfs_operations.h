...
typedef struct indirectionBlock {
    struct simpleBlock *blocks[BLOCKSIZE / sizeof(struct simpleBlock *)];
} indirectionBlock;

typedef struct linkedBlock {
    struct linkedBlock *next;
    uint8_t data[BLOCKSIZE - sizeof(struct linkedBlock *)];
} linkedBlock;
...
typedef struct inode {
    ...
    uint32_t num_blocks;        // num blocks for file

    struct timespec st_atim;    // time of last access
    struct timespec st_mtim;    // time of last modification
    struct timespec st_ctim;    // time of last status change

    struct simpleBlock *directBlocks[10];

    struct indirectionBlock *indirectionBlocks[68];

    struct linkedBlock *linkedStart;
    struct linkedBlock *linkedEnd;
    struct linkedBlock *linkedRecent;
} inode;
